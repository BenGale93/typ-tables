...
class TestEdgeCases:
    ...
    def test_default_row_group_cell_style_none(self, table_check, group_data):
        table = TypTable(group_data, rowname_col="fruit", groupname_col="group")

        table._typ_data.default_styles.row_group.cell = None
        result = table.to_typst()
    ...
    def test_default_row_group_stroke_none(self, table_check, group_data):
        table = TypTable(group_data, rowname_col="fruit", groupname_col="group")

        assert table._typ_data.default_styles.row_group.cell is not None
        table._typ_data.default_styles.row_group.cell.stroke = None
        result = table.to_typst()
    ...
    def test_default_row_group_stroke_set(self, table_check, group_data):
        table = TypTable(group_data, rowname_col="fruit", groupname_col="group")

        assert table._typ_data.default_styles.row_group.cell is not None
        table._typ_data.default_styles.row_group.cell.stroke = "2pt"
        result = table.to_typst()

        assert result == external("uuid:5d5b7152-522c-4366-841a-93db03396c3c.typ")
