...
class TestResolveColumns:
    def test_resolve_cols_polars_expr(self, col_data_polars):
        assert location.resolve_columns(col_data_polars, ncs.matches("[ab]")) == [("a", 0), ("b", 1)]

    def test_resolve_cols_polars_int(self, col_data_polars):
        assert location.resolve_columns(col_data_polars, -1) == [("c", 2)]

    def test_resolve_cols_pandas_list(self, col_data_pandas):
        assert location.resolve_columns(col_data_pandas, ["a", "c"]) == [("a", 0), ("c", 2)]

    def test_resolve_cols_pandas_str(self, col_data_pandas):
        assert location.resolve_columns(col_data_pandas, "a") == [("a", 0)]

    def test_resolve_cols_pandas_int_list(self, col_data_pandas):
        assert location.resolve_columns(col_data_pandas, [0, -1]) == [("a", 0), ("c", 2)]
